...
const districts = Object.values(locations.districts_bn).flat();
const upazilas = Object.values(locations.upazilas_bn).flat();
const unions = Object.values(locations.unions_bn).flat();

export default function Home() {
  const divisionStats = locations.divisions_bn.map((d) => ({
    ...d,
    districtCount: (locations.districts_bn[d.value] || []).length,
  }));
      ...
      <div className="ps-content">
        ...
        <form action="/search" className="ps-search">
          ...
          <input
            name="q"
            type="search"
            placeholder="ব্যবসা, সেবা, প্রতিষ্ঠান বা এলাকা খুঁজুন..."
            aria-label="Search"
          />
        </form>
          ...
          {categories.map((c) => (
            <a key={c.id} href={"/services?category=" + c.id} className="ps-cat-card">
              ...
              <div className="ps-cat-count">
                {demoCountByCategory(c.id) ? demoCountByCategory(c.id) + " টি ডেমো" : "তথ্য যোগ করুন"}
              </div>
            </a>
          ))}
        </div>
        ...
        <div className="ps-section-head">
          <span className="ps-section-icon">➕</span>
          <h2>আপনার এলাকার তথ্য যোগ করুন</h2>
        </div>
